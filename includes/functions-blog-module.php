 ...
 */
function extra_child_theme_blog_setup() {

	add_action( 'wp', 'extra_child_theme_blog_setup2', 9999 );


	function extra_child_theme_blog_setup2() {
		if ( class_exists( 'ET_Builder_Module' ) ) {

			class photoblog_ET_Builder_Module_Blog2 extends ET_Builder_Module_Type_PostBased {
				function init() {
				...
				function shortcode_callback( $atts, $content = null, $function_name ) {
					...
					if ( 'on' !== $fullwidth ) {
						$output = sprintf( '<div%5$s class="et_pb_module et_pb_blog_grid_wrapper%6$s">
				...
				<div class="et_pb_ajax_pagination_container">
					...
					%4$s %12$s
				</div>', ( 'on' === $fullwidth ? 'et_pb_posts' : 'et_pb_blog_grid clearfix' ), $posts, esc_attr( $class ), ( ! $container_is_closed ? '</div> <!-- .et_pb_posts -->' : '' ), ( '' !== $module_id ? sprintf( ' id="%1$s"', esc_attr( $module_id ) ) : '' ), ( '' !== $module_class ? sprintf( ' %1$s', esc_attr( $module_class ) ) : '' ), '' !== $video_background ? ' et_pb_section_video et_pb_preload' : '', $video_background, '' !== $parallax_image_background ? ' et_pb_section_parallax' : '', $parallax_image_background, $this->get_text_orientation_classname(), $this->drop_shadow_back_compatibility( $function_name ) );
					} else {
						$output = sprintf( '<div%5$s class="et_pb_module %1$s%3$s%6$s%7$s%9$s%11$s">
				%10$s
				%8$s
				<div class="et_pb_ajax_pagination_container">
					%2$s
				</div>
				%4$s %12$s', ( 'on' === $fullwidth ? 'et_pb_posts' : 'et_pb_blog_grid clearfix' ), $posts, esc_attr( $class ), ( ! $container_is_closed ? '</div> <!-- .et_pb_posts -->' : '' ), ( '' !== $module_id ? sprintf( ' id="%1$s"', esc_attr( $module_id ) ) : '' ), ( '' !== $module_class ? sprintf( ' %1$s', esc_attr( $module_class ) ) : '' ), '' !== $video_background ? ' et_pb_section_video et_pb_preload' : '', $video_background, '' !== $parallax_image_background ? ' et_pb_section_parallax' : '', $parallax_image_background, $this->get_text_orientation_classname(), $this->drop_shadow_back_compatibility( $function_name ) );
					}
				...
				protected function _add_additional_border_fields() {
					...
					$this->_additional_fields_options = array_merge( $this->_additional_fields_options, ET_Builder_Module_Fields_Factory::get( 'Border' )->get_fields( array(
						'suffix'          => "_{$suffix}",
						'depends_to'      => array( 'fullwidth' ),
						'depends_show_if' => 'on',
						'defaults'        => array(
		...
		$cgm2 = new photoblog_ET_Builder_Module_Blog2();
		remove_shortcode( 'et_pb_blog' );
		add_shortcode( 'et_pb_blog', array( $cgm2, '_shortcode_callback' ) );
