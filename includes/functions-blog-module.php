...
function extra_child_theme_blog_setup() {
	if ( class_exists( 'ET_Builder_Module' ) ) {
		class photoblog_ET_Builder_Module_Blog extends ET_Builder_Module {
			function init() {
			...
			function get_fields() {
				$fields = array(
					'fullwidth'                     => array(
						'label'           => esc_html__( 'PB Layout', 'et_builder' ),
						'type'            => 'select',
			...
			function shortcode_callback( $atts, $content = null, $function_name ) {
				...
				*/
				...
				/**
				 * Russells fix for image croping......
				 */

				//rone_debug_print_image_sizes();
				//die();
		...
		//
		$cgm = new photoblog_ET_Builder_Module_Blog();
		remove_shortcode( 'et_pb_blog' );
		add_shortcode( 'et_pb_blog', array( $cgm, '_shortcode_callback' ) );

	}
}

//add_action( 'wp', 'extra_child_theme_blog_setup', 9999 );
	...
	if ( class_exists( 'ET_Builder_Module' ) ) {
		...
		class photoblog_ET_Builder_Module_Blog2 extends ET_Builder_Module_Type_PostBased {
			...
			function shortcode_callback( $atts, $content = null, $function_name ) {
				...
				$output = sprintf( '<div%5$s class="%1$s%3$s%6$s%7$s%9$s%11$s">
				%10$s
				%8$s
				<div class="et_pb_ajax_pagination_container">
					%2$s
				</div>
			%4$s', ( 'on' === $fullwidth ? 'et_pb_posts' : 'et_pb_blog_grid clearfix' ), $posts, esc_attr( $class ), ( ! $container_is_closed ? '</div> <!-- .et_pb_posts -->' : '' ), ( '' !== $module_id ? sprintf( ' id="%1$s"', esc_attr( $module_id ) ) : '' ), ( '' !== $module_class ? sprintf( ' %1$s', esc_attr( $module_class ) ) : '' ), '' !== $video_background ? ' et_pb_section_video et_pb_preload' : '', $video_background, '' !== $parallax_image_background ? ' et_pb_section_parallax' : '', $parallax_image_background, $this->get_text_orientation_classname() );
